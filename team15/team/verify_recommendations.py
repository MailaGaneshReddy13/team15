...
if not Course.objects.exists():
    ...
    Course.objects.create(title="Kubernetes Mastery", description="Orchestration with K8s", category="technical")
...
# Test 2: Missing skills with no direct match (fallback)
missing_skills_fallback = ["NonExistentSkill"]
print(f"Testing with missing skills (fallback): {missing_skills_fallback}")
courses_fallback = get_recommended_courses(missing_skills_fallback)
print(f"Recommended Courses Fallback ({len(courses_fallback)}): {[c.title for c in courses_fallback]}")
